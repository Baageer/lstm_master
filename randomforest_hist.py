...
savafilename = "srcdata10_5"

s_list = ['600297', '600170', '600219', '600369', '600372',
          '600486', '600511', '600583', '600612', '600132', '600128']
...
data_dict = {}
for item in s_list:
    data = ts.get_hist_data(item, start="2018-01-01", end="2020-5-31")
    
    data = data.reindex(index=data.index[::-1])

    data1 = ts.get_hist_data(item, start="2020-06-01", end="2020-11-15")
...
for idx,item in enumerate(s_list):
    ...
    while(True):
        ...
        temp_y1 = train_dataset_y[idx_start+TIME_STEPS+1]
        temp_y2 = train_dataset_y[idx_start+TIME_STEPS+2]
        temp_y3 = train_dataset_y[idx_start+TIME_STEPS+3]
        temp_y = np.hstack((temp_y1, temp_y2, temp_y3))

        save_train.append({"x":temp_x, "y":temp_y})

        idx_start += 1
        if idx_start+TIME_STEPS+3 == train_length:
            break
    idx_start = 0
    while(True):
        temp_x = test_dataset_x[idx_start:idx_start+TIME_STEPS]
        temp_y1 = test_dataset_y[idx_start+TIME_STEPS+1]
        temp_y2 = test_dataset_y[idx_start+TIME_STEPS+2]
        temp_y3 = test_dataset_y[idx_start+TIME_STEPS+3]
        temp_y = np.hstack((temp_y1, temp_y2, temp_y3))

        save_test.append({"x":temp_x, "y":temp_y})

        idx_start += 1
        if idx_start+TIME_STEPS+3 == test_length:
            break
